...
void MultiadapterTwit::LoadPipeline()
{
...
void MultiadapterTwit::LoadAssets()
{
    ...
    {
        {
            const UINT64 constantBufferSize = sizeof(SceneConstantBuffer) * MaxBirdCount * FrameCount;
...
float MultiadapterTwit::GetRandomFloat(float min, float max)
{
...
void MultiadapterTwit::OnUpdate()
{
            ...
            // Adjust the render time to be greater than the blur time.
            {
                ...
                if (timeDelta < -.10f || timeDelta > .01f)
                {
                    if (AllowDrawDynamicWorkload)
                    {
                        ...
                        const float stepSize = max(1.0f, m_BirdCount);
                        m_BirdCount = min(m_BirdCount + static_cast<INT>(stepSize * timeDelta), MaxBirdCount);
                    }
    ...
    // Update the triangles.
    {
        ...
        SceneConstantBuffer* dst = m_pCbvDataBegin + (m_frameIndex * MaxBirdCount);
        memcpy(dst, &m_constantBufferData[0], m_BirdCount * sizeof(SceneConstantBuffer));
    }
}
...
void MultiadapterTwit::OnDestroy()
{
...
void MultiadapterTwit::PopulateCommandLists()
{
    // Command list to render target the triangles on the primary adapter.
    {
        ...
        for (UINT n = 0; n < m_BirdCount; n++)
        {
            const D3D12_GPU_VIRTUAL_ADDRESS cbLocation = cbVirtualAddress + (m_frameIndex * MaxBirdCount * sizeof(SceneConstantBuffer)) + (n * sizeof(SceneConstantBuffer));
...
void MultiadapterTwit::UpdateWindowTitle()
{
    std::wstringstream stringStream;

    stringStream << L"[" << m_BirdCount << L" triangles]";
...
void MultiadapterTwit::WaitForGpu(GraphicsAdapter adapter)
{
...
void MultiadapterTwit::MoveToNextFrame()
{
